...
<template>
  <div
      ...
      <div v-if="hasAction" class="vx-card__actions">
        <slot name="actions">
          ...
          <div v-if="codeToggler && !actionButtons" class="vx-card__code-toggler sm:block hidden">
            <feather-icon :class="{'border border-solid border-primary border-t-0 border-r-0 border-l-0': showCode}" icon="CodeIcon" @click="toggleCode"/>
          </div>
        </slot>
      </div>
    </div>

    <div ref="content" :class="[{collapsed: isContentCollapsed}, {'overflow-hidden': tempHidden}]" :style="StyleItems" class="vx-card__collapsible-content vs-con-loading__container">
    ...
    <div v-show="this.$slots.codeContainer" ref="codeContainer" :style="codeContainerStyles" :class="{collapsed: !showCode}" class="vx-card__code-container">
